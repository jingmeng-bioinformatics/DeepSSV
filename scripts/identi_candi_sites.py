...
parser.add_argument('--Tumor_Normal_mpileup', required=True, metavar='pileup', help='a compressed mixed mpileup file from tumor and normal bam files generated by samtools')
parser.add_argument('--Candidate_somatic_sites', required=True, metavar='Information_about_candidate_sites', help='mapping information about candidate somatic sites')
...
def process_line(line_content, line, Cs):
   ...
   # keep just the genomics sites that meet the criteria
    if line_content[2] not in 'Nn' and int(line_content[3]) >= 10 and int(line_content[8]) >= 10 and sum([len(re.findall(base, line_content[4])) for base in pattern]) > 0:
        ...
        if max_index == 5 and length_of_inserted_bases > 50:
            ...
            if (0.2 < (allele_forward_count[max_index]/max(allele_count)) < 0.8 and (max(allele_count)/int(line_content[3])) >= 0.075 and
...
def main(args):
    with open(args.Tumor_Normal_mpileup, 'rt') as TN, open(args.Candidate_somatic_sites, 'wt') as Cs:
        n = 110
        ...
        for line, line_pair in combi:
            ...
            if line_content[0] == line_pair_content[0] and (line_pair_content[1] - line_content[1] == n):
